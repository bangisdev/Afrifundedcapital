...
- Name: audit-logs.spec.ts >> 11. Purchase label & audit trail — audit log chips & lifecycle >> audit log chips render and filter the lifecycle + payment actions
- Location: e2e/audit-logs.spec.ts:46:3
...
Locator: getByRole('button', { name: 'Funded' })
Expected: visible
Error: strict mode violation: getByRole('button', { name: 'Funded' }) resolved to 2 elements:
    1) <button type="button" aria-pressed="false" class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-medium transition-colors border-border text-muted-foreground hover:text-foreground hover:border-foreground/40">…</button> aka getByRole('button', { name: 'Funded', exact: true })
    2) <button type="button" aria-pressed="false" class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-medium transition-colors border-border text-muted-foreground hover:text-foreground hover:border-foreground/40">…</button> aka getByRole('button', { name: 'Refunded' })

Call log:
  - Expect "toBeVisible" with timeout 15000ms
  - waiting for getByRole('button', { name: 'Funded' })
