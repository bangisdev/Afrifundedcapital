...
export const resetChallengeDemoData = action({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    if (!challenge) {
      return { success: false, message: "Challenge not found or not eligible for demo data." };
    }
    ...
    for (let day = totalDays - 1; day >= 0; day--) {
      ...
      try {
        await (ctx as any).runMutation((internal as any).trading.recordTradingMetrics, {
          ...
          trailingDrawdown: Math.round(currentDrawdown * 100) / 100,
          relativeDrawdown: Math.round(currentDrawdown * 100) / 100,
          absoluteDrawdown: Math.round(Math.max(0, accountSize - currentBalance) / accountSize * 10000) / 100,
    ...
    return {
      success: true,
      message: `Reset and reseeded ${tradingDayCount} data points for this challenge.`,
    };
  },
});
